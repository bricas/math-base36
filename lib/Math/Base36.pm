...
sub decode_base36 {
    my $base36 = uc( shift );
    die "Invalid base36 number ($base36)" if $base36 =~ m{[^0-9A-Z]};
...
sub encode_base36 {
    ...
    die "Invalid base10 number ($number)"  if $number    =~ m{\D};
    die "Invalid padding length ($padlength)" if $padlength =~ m{\D};
